...
/* HTC_AUD_START */
static int tfa98xx_mixer_profile = -1/* nxp:0 */; /* current mixer profile */
static unsigned char cf_tfa9891_bypass[3][3] = {
	{0x09, 0x82, 0x68}, /* bypass and power up */
	{0x09, 0x82, 0x69}, /* bypass and power down */
	{0x04, 0x88, 0x5B}
};
/* HTC_AUD_END */
...
static void __tfa98xx_inputdev_check_register(struct tfa98xx *tfa98xx, bool unregister)
{
	...
	for (i = 0; i < tfaContMaxProfile(tfa98xx->handle); i++) {
		if (strnstr(tfaContProfileName(tfa98xx->handle, i), ".tap", strlen(tfaContProfileName(tfa98xx->handle, i)))) {
			tap_profile = true;
...
/* HTC_AUD_START */
	if (tfa98xx_mixer_profile == -1) {
		tfa98xx_write_raw(tfa98xx->handle, 3, cf_tfa9891_bypass[2]);
		tfa98xx_write_raw(tfa98xx->handle, 3, cf_tfa9891_bypass[(mute)?1:0]);
		pr_err("tfa9891 bypass mode and power %s", (mute)? "down":"up");
		return 0;
	}
/* HTC_AUD_END */
