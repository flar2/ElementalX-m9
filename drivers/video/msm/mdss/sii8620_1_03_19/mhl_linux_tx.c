			 ...
			 const char *buf, size_t count)
{
	...
	uint16_t burst_id;
	char *pinput = kzalloc(count + 1, GFP_KERNEL);
			 ...
			 const char *buf, size_t count)
{
	...
	if (-ERANGE == status) {
		...
		MHL_TX_DBG_ERR("temp:0x%lx buf:%s%s%s\n", temp,
			ANSI_ESC_RED_TEXT, buf, ANSI_ESC_RESET_TEXT);
	} else {
		if (down_interruptible(&dev_context->isr_lock)) {
			MHL_TX_DBG_ERR("%scould not get mutex%s\n",
...
struct device_attribute driver_attribs[] = {
	...
	__ATTR(SYS_ATTR_NAME_SII6031_CTL, 0200, NULL, set_sii6031_ctl),
#endif
...
static struct device_attribute rbp_out_attr =
	__ATTR(SYS_ATTR_NAME_RBP_OUT, 0644, show_rbp_out, send_rbp_out);
				...
				struct i2c_client *client)
{
	...
	dev_context->mhl_dev = device_create(mhl_class, &client->dev,
	...
	if (ret) {
		MHL_TX_DBG_ERR("Failed to acquire ISR semaphore, status: %d\n", ret);
		goto free_device;
	}
...
free_irq_handler:
	if (client)
		free_irq(client->irq, dev_context);
