		...
		for (i = 0; i < limit - 1; ++i) {
			...
			if ((0 != p_desc->dtd.pixel_clock_low) ||
				...
				if ((0 == p_desc->dtd.horz_active_7_0) &&
					...
					uint8_t j;
					/* need to stop if this is the last one in the dtd list */
					/* otherwise we will fall into a infinite loop because we keep copying the last one */
					if (i >= limit - 1 - number_that_we_pruned) {
				...
				MHL_TX_EDID_INFO(
					"pix clock zero p_desc:%p", p_desc)
				p_desc++;
				MHL_TX_EDID_INFO("p_desc:%p\n", p_desc)
			}
		}
...
int si_mhl_tx_get_num_cea_861_extensions(void *context, uint8_t block_number)
{
	...
	if (!si_mhl_tx_do_edid_checksum(pb_data)) {
		MHL_TX_DBG_ERR("%sEDID -> Checksum Error pb_data:%p%s\n",
	...
	struct edid_3d_data_t *mhl_edid_3d_data, struct MHL_high_low_t hev_fmt)
{
	...
	if (NULL == mhl_edid_3d_data->hev_dtd_list) {
		MHL_TX_DBG_ERR("No HEV DTDs available: %x\n", index)
