...
void display_timing_enumeration_end(struct edid_3d_data_t *mhl_edid_3d_data)
{
	...
	if (NULL == mhl_edid_3d_data->hev_dtd_list) {
		...
		MHL_TX_DBG_WARN
		    ...
		    for (hev_dtd_index = 0, _3d_dtd_index =
			 mhl_edid_3d_data->parse_data.num_vesa_timing_dtds +
			 mhl_edid_3d_data->parse_data.num_cea_861_timing_dtds;
			 (hev_dtd_index <
			...
			MHL_TX_DBG_WARN
			    ("hev_dtd_index:0x%x _3d_dtd_index:0x%x\n",
			     hev_dtd_index, _3d_dtd_index)
			    mhl_edid_3d_data->hev_dtd_list[hev_dtd_index].
			    _3d_info =
			    mhl_edid_3d_data->_3d_dtd_list[_3d_dtd_index++].
			    _3d_info;
		}
	}
}
...
static int validate_timings_table(void)
{
	int match_count = 0;
	int i;
	for (i = 0; i < ARRAY_SIZE(timing_modes_from_data_sheet); ++i) {
		int j;
		for (j = i + 1; j < ARRAY_SIZE(timing_modes_from_data_sheet);
		     ++j) {
			int difference;
			if (timing_modes_from_data_sheet[i].h_total !=
			    timing_modes_from_data_sheet[j].h_total) {
				continue;
			}
			if (timing_modes_from_data_sheet[i].v_total !=
			    timing_modes_from_data_sheet[j].v_total) {
				continue;
			}
			if (timing_modes_from_data_sheet[i].columns !=
			    timing_modes_from_data_sheet[j].columns) {
				continue;
			}
			if (timing_modes_from_data_sheet[i].rows !=
			    timing_modes_from_data_sheet[j].rows) {
				continue;
			}
			difference =
			    timing_modes_from_data_sheet[i].field_rate -
			    timing_modes_from_data_sheet[j].field_rate;
			...
			MHL_TX_DBG_ERR
			    ...
			    "\tmatches entry %d:\n\t{%d,%d,%d,%d}%s\n",
			     ANSI_ESC_RED_TEXT, i,
			     timing_modes_from_data_sheet[i].h_total,
			     timing_modes_from_data_sheet[i].v_total,
			     timing_modes_from_data_sheet[i].columns,
			     timing_modes_from_data_sheet[i].rows, j,
			     timing_modes_from_data_sheet[j].h_total,
			     timing_modes_from_data_sheet[j].v_total,
			     timing_modes_from_data_sheet[j].columns,
			     timing_modes_from_data_sheet[j].rows,
	...
	struct si_incoming_timing_t *p_timing)
{
	...
	for (i = 0; i < ARRAY_SIZE(timing_modes_from_data_sheet); ++i) {
		...
		if (timing_modes_from_data_sheet[i].rows != p_timing->rows)
			continue;
		...
		p_timing->mhl3_vic = timing_modes_from_data_sheet[i].mhl3_vic;
		MHL_TX_DBG_ERR("%svic: %d %s%s\n", ANSI_ESC_GREEN_TEXT,
			       timing_modes_from_data_sheet[i].mhl3_vic,
...
static struct VIC_info_t VIC_info[] = {
	...
	CEA_861_F_VIC_info_entry(63, 1920, 1080, 280, 45, 120000,
		cea_iar_16_to_9, vsm_progressive, par_1_to_1,
		vif_dual_frame_rate, 0, 0),
	CEA_861_F_VIC_info_entry(64, 1920, 1080, 280, 45, 100000,
		cea_iar_16_to_9, vsm_progressive, par_1_to_1,
		vif_single_frame_rate, 0, 0),
	...
	if (ne_SUCCESS == si_mhl_tx_drv_get_edid_fifo_partial_block(
		...
		if (!si_mhl_tx_check_edid_header(mhl_edid_3d_data, p_EDID)) {
			...
			DUMP_EDID_BLOCK(1, pb_data,
				...
				EDID_block_data[1])) {
				MHL_TX_DBG_ERR(
				    "EDID header misaligned, pb_data: %p\n", pb_data);
				return ne_BAD_HEADER_OFFSET_BY_1;
			} else {
				int ret_val;
				MHL_TX_DBG_ERR(
				    "sre-reading buffer, pb_data: %p\n", pb_data);
				...
				ret_val =
				    ...
				    mhl_edid_3d_data->drv_context,
				     pb_data);
				if (ne_SUCCESS != ret_val) {
					MHL_TX_DBG_ERR("loss of HPD, pb_data: %p",
						       pb_data);
					return ret_val;
				} else
				    if (!si_mhl_tx_check_edid_header(
...
int si_mhl_tx_get_num_cea_861_extensions(void *context, uint8_t block_number)
{
	...
	if (!si_mhl_tx_do_edid_checksum(pb_data)) {
		MHL_TX_DBG_ERR("EDID -> Checksum Error pb_data: %p\n",
			pb_data);
