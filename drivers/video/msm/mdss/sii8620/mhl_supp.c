					     ...
					     *dev_context)
{
	...
	MHL_TX_DBG_INFO("high: %d, low: %d\n", req->payload->hdr_and_burst_id.burst_id.high, req->payload->hdr_and_burst_id.burst_id.low);
...
void si_mhl_tx_push_block_transactions(struct mhl_dev_context *dev_context)
{
	...
	MHL_TX_DBG_INFO
	    ("=== sub_payload_size: %d, ack count: %d, queue empty %d\n",
	     req->sub_payload_size, ack_byte_count,
	...
	while (!list_empty(&dev_context->block_protocol.queue)) {
		...
		if (hw_context->block_protocol.peer_blk_rx_buffer_avail <
		    payload_size) {
		...
		hw_context->block_protocol.peer_blk_rx_buffer_avail -=
		    payload_size;
		MHL_TX_DBG_WARN("WR PEER Buffer Available: %ld\n",
				hw_context->block_protocol.
				peer_blk_rx_buffer_avail);
...
void si_mhl_tx_send_blk_rcv_buf_info(struct mhl_dev_context *dev_context)
{
	...
	if (NULL == blk_rcv_buf_info) {
		...
		MHL_TX_DBG_WARN
		    ("blk_rcv_buffer_info: id:0x%02x%02x size: 0x%02x%02x\n",
		     blk_rcv_buf_info->burst_id.high,
		     blk_rcv_buf_info->burst_id.low,
		     blk_rcv_buf_info->blk_rcv_buffer_size_high,
		     blk_rcv_buf_info->blk_rcv_buffer_size_low);
			  ...
			  bool xstat, uint8_t reg_to_write, uint8_t value)
{
	...
	if (req == NULL) {
		MHL_TX_DBG_INFO("CBUS free queue exhausted\n");
		    return false;
	}
...
void si_mhl_tx_process_events(struct mhl_dev_context *dev_context)
{
	...
	if (dev_context->mhl_connection_event) {
		...
		if (MHL_TX_EVENT_DISCONNECTION == dev_context->mhl_connected) {
			MHL_TX_DBG_WARN
			    ("MHL Disconnection Event. Reset states.\n");
			si_mhl_tx_reset_states(dev_context);
		}
		...
		case MHL_MSC_MSG_RAPK:
			...
			if (MHL_RAP_CBUS_MODE_DOWN ==
				...
				MHL_TX_DBG_ERR("unexpected outgoing RAP "
					"command: 0x%02x\n",
					dev_context->msc_msg_last_data);
			}
			break;
	...
	if (req->status.flags.cancel == true) {
		...
		if (XDEVCAP_ADDR_ECBUS_SPEEDS == req->reg) {
			...
			if (si_mhl_tx_drv_get_cbus_mode(dev_context) <
				 ...
				 * RAP support is required for MHL3 and later
				 * devices
				 */
				       ...
				       uint8_t *write_burst_data)
{
	...
	if ((can_reassign)
	    && (0 ==
		si_mhl_tx_drv_set_tdm_slot_allocation(
			(struct drv_hw_context *)&dev_context->drv_context,
