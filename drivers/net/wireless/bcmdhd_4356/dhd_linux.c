 ...
 *
 * $Id: dhd_linux.c 599022 2015-11-12 03:30:40Z $
 */
...
/* HTC_WIFI_END */
#if defined(USE_STATIC_MEMDUMP) && !defined(CUSTOMER_HW_ONE)
...
#endif /* defined(USE_STATIC_MEMDUMP) && !defined(CUSTOMER_HW_ONE) */
...
static void dhd_os_wake_lock_init(struct dhd_info *dhd);
static void dhd_os_wake_lock_destroy(struct dhd_info *dhd);
...
#endif /* DHD_UNICAST_DHCP || DHD_L2_FILTER */
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	if (dhd->up) {
		if (value && dhd->in_suspend) {
				...
				/* KDDI APK no need active mode in screen off */
...
#ifdef CUSTOMER_HW_ONE
	/* Framework will issue suspend cmd skip it */
	return;
#endif
	if (dhd)
...
#endif /* defined(SOFTAP) && !defined(CUSTOMER_HW_ONE) */
...
#endif /* DHD_UNICAST_DHCP */
#ifdef DHD_L2_FILTER
	/* If trace_ping is enabled parse the ping packet */
	if (dhdp->trace_ping) {
		if (dhd_l2_filter_block_ping(dhdp, pktbuf, ifidx) == BCME_OK) {
			/* dump in dhd_l2_filter_block_ping, do nothing */
		}
	}
#endif
...
/* HTC_WIFI_END */
	...
	if (ret == BCME_NORESOURCE) {
		...
		if (txq_full_event_num >= MAX_TXQ_FULL_EVENT) {
			...
			DHD_ERROR(("%s : over MAX_TXQ_FULL_EVENT\n", __FUNCTION__));
		}
	}
	else {
		txq_full_event_num = 0;
...
#else
			DHD_ERROR_HW_ONE(("ETHER_TYPE_802_1X [RX]: "
				"ver %d, type %d, replay %d\n",
#endif
/* HTC_WIFI_END */
...
/* Add debug log for rx interrupt  */
		...
		if (ntoh16(skb->protocol) == ETHER_TYPE_BRCM) {
			...
			/* only do rest work when evt_ret is OK */
			if (!evt_ret) {
...
/* HTC_WIFI_END */
			...
			if (event.event_type == WLC_E_PFN_NET_FOUND) {
				...
				/* dump packet content, if evt_ret is not ok. */
...
#if defined(DHD_USE_IDLECOUNT) && defined(BCMPCIE)
		if (dhd_bus_wakeup(pub)) {
			DHD_ERROR(("%s : runtime resume done \n", __FUNCTION__));
		} else {
#endif /* DHD_USE_IDLECOUNT && BCMPCIE */
...
#if defined(DHD_USE_IDLECOUNT) && defined(BCMPCIE)
	/* Runtime Resume is needed before interface down */
	dhd_bus_wakeup(&dhd->pub);
#endif /* DHD_USE_IDLECOUNT && BCMPCIE */
...
#define DEFAULT_LOG_STR_PATH "/system/etc/firmware/logstrs.bin"
#else
...
#endif /* OEM_ANDROID && DEBUGABILITY_SUPPORT */
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
...
#ifdef CUSTOMER_HW_ONE
	/* Print out cis source for debug */
	{
		int cis_source = 0;
	...
	/* Force WLC_DOWN for commands requires DOWN such as lpc, ampdu_hostreorder */
...
#if defined(HTC_TX_TRACKING)
{
	...
	uint tx_stat_chk_num = 5; /* check only if there is more than 5 packet send in period */
	...
	if ((dhd->op_mode & DHD_FLAG_HOSTAP_MODE) ||
	    (dhd->op_mode & DHD_FLAG_MFG_MODE)) {
		dhd_bus_idleclock(dhd, DHD_IDLE_ACTIVE);
	}
#endif /* DHD_USE_IDLEACTIVE */
	/* Set Country code  */
...
dhd_register_if(dhd_pub_t *dhdp, int ifidx, bool need_rtnl_lock)
{
	...
	/* HTC_WIFI_START */
	// ** 20150216 Solve MAC security leakage
...
#ifdef DHD_USE_IDLEACTIVE
int
dhd_net_bus_idleclock(struct net_device *dev, uint32 flag)
{
	int ret = 0;
	dhd_info_t *dhd = DHD_DEV_INFO(dev);

	DHD_ERROR(("%s: dev %p dhd %p flag %d\n", __FUNCTION__, dev, dhd, flag));
	ret = dhd_bus_idleclock(&dhd->pub, flag);
	if (ret) {
		DHD_ERROR(("%s: dhd_bus_idleclock: %d\n", __FUNCTION__, ret));
		return ret;
	}
...
int net_os_set_suspend(struct net_device *dev, int val, int force)
{
	...
	/* +++ 20130624 wright, release performance lock when set suspend */
#ifdef CUSTOMER_HW_ONE
	if (val == 1) {
...
#else
	return 0;
#endif /* !defined(CUSTOMER_HW_ONE) && !defined(PCIE_FULL_DONGLE) */
}
...
static void dhd_os_wake_lock_init(struct dhd_info *dhd)
{
        DHD_ERROR(("%s: initialize wake_lock_counters\n", __FUNCTION__));
...
static void dhd_os_wake_lock_destroy(struct dhd_info *dhd)
{
        DHD_ERROR(("%s: deinit wake_lock_counters\n", __FUNCTION__));
#ifdef CONFIG_HAS_WAKELOCK
        dhd->wakelock_counter = 0;
        dhd->wakelock_wd_counter = 0;
        dhd->wakelock_rx_timeout_enable = 0;
		...
		if ((current->on_cpu > 0) && !rxf_rt_class(dhd_rxf_prio)) {
			/* HTC_WIFI_START */
			{
				...
				if (sched_setaffinity(0, &mask) < 0) {
					/* DHD_ERROR(("sched_setaffinity failed")); */
				}
				else {
...
{
	dhd_info_t *dhd = *(dhd_info_t **)netdev_priv(dev);
	/* DHD_ERROR(("%s: add: %d, id: %d, offset: %d, mask: %s, pattern: %s\n", __func__,
...
int dhdhtc_update_wifi_power_mode(int is_screen_off)
{
	...
	if (dhdhtc_power_ctrl_mask) {
		...
		/* 2014-10-23 Hugh change the code to fit the new drv code design */
	...
	/* } else if (dhdcdc_power_active_while_plugin && usb_get_connect_type()) {
		DHD_ERROR(("power active. usb_type:%d\n", usb_get_connect_type()));
		pm_type = PM_OFF;
...
int dhd_os_get_socram_dump(struct net_device *dev, char **buf, uint32 *size)
{
	...
	if (buf == NULL)
		return BCME_ERROR;
	orig_len = *size;
	if (dhdp->soc_ram && dhdp->soc_ram_length) {
