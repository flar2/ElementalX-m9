...
int osl_static_mem_init(osl_t *osh, void *adapter)
{
	...
	if (!bcm_static_buf && adapter) {
		if (!(bcm_static_buf = (bcm_static_buf_t *)wifi_platform_prealloc(adapter,
			...
			printk("[WLAN] can not alloc static buf!\n");
			bcm_static_skb = NULL;
			ASSERT(osh->magic == OS_HANDLE_MAGIC);
			return -ENOMEM;
		}
		else
			printk("[WLAN] alloc static buf at %p!\n", bcm_static_buf);
	...
	if (!bcm_static_skb && adapter) {
		...
		if (!skb_buff_ptr) {
			printk("[WLAN] cannot alloc static buf!\n");
			bcm_static_buf = NULL;
			bcm_static_skb = NULL;
			ASSERT(osh->magic == OS_HANDLE_MAGIC);
			return -ENOMEM;
...
#endif /* MEM_DEBUG_PAGEALLOC */
			printk("[WLAN] %s: %d skb %p min %p max %p\n",
				__FUNCTION__, i,
				bcm_static_skb->skb_8k[i],
				bcm_static_skb->min,
				bcm_static_skb->max);
			if (bcm_static_skb->skb_8k[i] < bcm_static_skb->min)
...
osl_pktfree(osl_t *osh, void *p, bool send)
{
	...
	if (skb && (skb->mac_len == PREALLOC_USED_MAGIC)) {
		printk("[WLAN] %s: Free Static %p not from pkt free static!\n", __FUNCTION__, p);
		dump_stack();
		return;
	}
...
#endif /* MEM_DEBUG && MEM_DEBUG_PAGEALLOC */
...
osl_pktget_static(osl_t *osh, uint len)
{
	...
	if (len > DHD_SKB_MAX_BUFSIZE) {
		printk("[WLAN] %s: attempt to allocate huge packet (0x%x)\n", __FUNCTION__, len);
		p = osl_pktget(osh, len);
#if defined(MEM_DEBUG)
		printk("[WLAN] %s: Get non-static p %p \n", __FUNCTION__, p);
#endif
		return p;
	}
...
pagealloc_done:
	...
	p = osl_pktget(osh, len);
#if defined(MEM_DEBUG)
	printk("[WLAN] %s: all static pkt in use! Get non-static p %p \n", __FUNCTION__, p);
#endif
	return p;
}
...
osl_pktfree_static(osl_t *osh, void *p, bool send)
{
	...
	for (i = 0; i < STATIC_PKT_2PAGE_NUM; i++) {
		if (p == bcm_static_skb->skb_8k[i]) {
			if (bcm_static_skb->pkt_use[i] == 0)
				printk("[WLAN] %s: double free! %d p %p\n", __FUNCTION__, i, p);
			bcm_static_skb->pkt_use[i] = 0;
#if defined(MEM_DEBUG)
	...
	/* try to call osl_pktfree to avoid skb buffer leak. */
	/* maybe some network packet has the same protocol type as event. */
	...
	up(&bcm_static_skb->osl_pkt_sem);
#if defined(MEM_DEBUG)
	printk("[WLAN] %s: Free non_static p %p\n", __FUNCTION__, p);
#endif
	osl_pktfree(osh, p, send);
#endif /* BCMPCIE && DHD_USE_STATIC_IOCTLBUF */
}
...
#if defined(MEM_DEBUG)
void
osl_pktchk_static(osl_t *osh, void *p)
{
	int i;
#if defined(BCMPCIE)
	unsigned long flags;
	...
	BCM_REFERENCE(osh);
	if (!bcm_static_skb) {
		printk("[WLAN] %s: bcm_static_skb NULL\n", __FUNCTION__);
		return;
	}
	...
	if ((struct sk_buff *)p < bcm_static_skb->min || (struct sk_buff*)p > bcm_static_skb->max) {
#if defined(BCMPCIE)
		spin_unlock_irqrestore(&bcm_static_skb->osl_pkt_lock, flags);
#else
		up(&bcm_static_skb->osl_pkt_sem);
	...
	for (i = STATIC_PKT_1PAGE_NUM; i < STATIC_PKT_1_2PAGE_NUM; i++) {
		if (p == bcm_static_skb->skb_8k[i - STATIC_PKT_1PAGE_NUM]) {
			if (bcm_static_skb->pkt_use[i] == 0) {
				printk("[WLAN] %s: %d p %p not in use\n", __FUNCTION__, i, p);
				dump_stack();
			}
#if defined(BCMPCIE)
			spin_unlock_irqrestore(&bcm_static_skb->osl_pkt_lock, flags);
...
#ifdef CUSTOMER_HW_ONE
		if (osh)
			printk("[WLAN] MALLOC_FAILED: failed=%d\n", osh->failed);
		WARN_ON(1);
#endif 
		return (NULL);
	}
...
#if (defined(BCM47XX_CA9) || defined(USE_KMALLOC_FOR_FLOW_RING)) && \
...
#endif /* (BCM47XX_CA9 || USE_KMALLOC_FOR_FLOW_RING) && __ARM_ARCH_7A__ */
