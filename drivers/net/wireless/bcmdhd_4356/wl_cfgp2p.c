		...
		if (wl_get_drv_status_all(cfg, REMAINING_ON_CHANNEL) ||
			...
			if (ndev && (ndev->ieee80211_ptr != NULL)) {
				...
				if (bcmcfg_to_p2p_wdev(cfg)) {
					...
					WL_ERR(("bcmcfg_to_p2p_wdev(cfg) is NULL,"
						"skip cfg80211_remain_on_channel_expired \n"));
				}
#else
				cfg80211_remain_on_channel_expired(bcmcfg_to_p2p_wdev(cfg),
					cfg->last_roc_id, &cfg->remain_on_chan, GFP_KERNEL);
...
wl_cfgp2p_del_p2p_disc_if(struct wireless_dev *wdev, struct bcm_cfg80211 *cfg)
{
	...
	/* cfg80211_unregister_wdev() has no synchronize_rcu() after list_del_rcu(). */
	/* We add the synchronize_rcu() here to make sure the list_del_rcu() inside  */
	/* cfg80211_unregister_wdev() is OK for all cpus. */
