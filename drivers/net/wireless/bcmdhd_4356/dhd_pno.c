...
{
/* HTC_WIFI_START */
// [Bug fix] Hotspot cannot be scan due to select dfs channel, avoid it.
// Remove original code
#if 0
	if (channel >= 52 && channel <= 64)			/* class 2 */
// Add new code
#else
	if (channel >= 50 && channel <= 64)			/* class 2 */
#endif
/* HTC_WIFI_END */
		return TRUE;
/* HTC_WIFI_START */
...
#ifdef PNO_DEBUG
{
	...
	for (i = 0; i < hotlist_params->nbssid; i++) {
		...
		if (_pno_bssid == NULL) {
			DHD_ERROR(("%s : failed to allocate struct dhd_pno_bssid\n",
				__FUNCTION__));
			err = BCME_NOMEM;
			goto exit;
		}
...
exit:
	...
	if (err < 0) {
		_params = &_pno_state->pno_params_arr[INDEX_OF_HOTLIST_PARAMS];
		_dhd_pno_reinitialize_prof(dhd, _params, DHD_PNO_HOTLIST_MODE);
		_pno_state->pno_mode &= ~DHD_PNO_HOTLIST_MODE;
	}
	return err;
}
