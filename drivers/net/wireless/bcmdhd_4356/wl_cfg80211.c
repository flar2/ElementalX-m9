 ...
 *
 * $Id: wl_cfg80211.c 586730 2015-09-16 07:33:07Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
static s32 wl_notify_txfail(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
...
int dhd_net_bus_idleclock(struct net_device *dev, uint32 flag);
#endif /* DHD_USE_IDLEACTIVE */
...
static s32 wl_notify_scan_status(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
	const wl_event_msg_t *e, void *data);
#ifdef REPORT_ROAM_DONE_EARLY
#define AS_ROAM_DONE 99
#endif
...
/* HTC_WIFI_START */
//** 20150630 Pop up message for OTP empty
#ifdef OTPDIALOG
	...
	if ((err = wldev_ioctl(dev, WLC_GET_BSSID, &bssid, sizeof(bssid), false))) {
		...
		if (cur_band == WLC_BAND_5G) {
			...
			if (chan > CHANNEL_2G_MAX) {
				...
				return wl_ch_host_to_driver(chan);
			}
		}
exit:
...
#if defined(DHD_USE_IDLEACTIVE) && defined(CUSTOMER_HW_ONE)
			dhd_net_bus_idleclock(primary_ndev, DHD_IDLE_ACTIVE);
#endif /* DHD_USE_IDLEACTIVE && CUSTOMER_HW_ONE */
	...
	if (cfg->p2p_supported) {
		...
		if (cfg->p2p->vif_created) {
			...
			if (ret != BCME_OK) {
				...
				if (!sta_connected) {
					struct net_device *ndev = bcmcfg_to_prmry_ndev(cfg);
					dhd_net_bus_idleclock(ndev, DHD_IDLE_STOP);
				}
#endif /* DHD_USE_IDLEACTIVE && CUSTOMER_HW_ONE */
			}
...
#ifdef OTPDIALOG
        if (!otp_write) {
            WL_DBG(("error - OTP is empty, send event notice upper layer\n"));
            wl_cfg80211_send_priv_event(ndev, "OTP_DIALOG");
        }
#endif
/* HTC_WIFI_END */
	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if (wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) {
		...
		if (memcmp(mac, curmacp, ETHER_ADDR_LEN)) {
			...
			mac_err_cnt++;
			if (mac_err_cnt >= 3) {
				WL_ERR(("Wrong Mac error count is over 3, send hang\n"));
		...
		/* HTC_WIFI_START */
		// ** [HPKB#7954] Check and fix abnormal RSSI when signal poll
		if ((rssi >= 0) || (rssi <= -110)) {
			WL_ERR(("RSSI %d dBm, fix it\n", rssi));
			rssi = -45;
		}
		/* HTC_WIFI_END */
...
#ifdef CUSTOMER_HW_ONE
	if ((ack == false) && cfg->afx_hdl->pending_tx_act_frm) {
		wl_action_frame_t *action_frame;
		action_frame = &(cfg->afx_hdl->pending_tx_act_frm->action_frame);
		if (wl_cfgp2p_is_p2p_gas_action(action_frame->data, action_frame->len)) {
			WL_DBG(("Failed to GAS send Action Frame.\n"));
			goto exit;
		}
	}
#endif /* CUSTOMER_HW_ONE */
	...
	struct bss_parameters *params)
{
	...
	s32 ap_isolate = AP_ISOLATE_SENDUP_ALL;
#else
	s32 ap_isolate = 0;
#endif
...
set_channel:
	...
	WL_DBG(("%s: chan %d bw 0x%x chspec 0x%x\n", __FUNCTION__, _chan, bw, chspec));
	if (wf_chspec_valid(chspec)) {
		fw_chspec = wl_chspec_host_to_driver(chspec);
		WL_DBG(("%s: chspec 0x%x fw_chspec 0x%x\n", __FUNCTION__, chspec, fw_chspec));
		...
		for (i = 0; i < request->n_match_sets; i++) {
			...
			if (ssid->ssid_len) {
				ssids_local[ssid_count].SSID_len = MIN(ssid->ssid_len,
						DOT11_MAX_SSID_LEN);
				memcpy(ssids_local[ssid_count].SSID, ssid->ssid, ssids_local[ssid_count].SSID_len);
...
#ifdef CONFIG_CFG80211_INTERNAL_REGDB
	wdev->wiphy->reg_notifier = (void *)wl_cfg80211_reg_notifier;
#endif /* CONFIG_CFG80211_INTERNAL_REGDB */
...
static s32 wl_inform_single_bss(struct bcm_cfg80211 *cfg, struct wl_bss_info *bi, bool roam)
{
	...
	if (!band) {
		...
		/* HTC_WIFI_START */
		//** Show current channel when get invalid band
		WL_ERR(("chanspec: %d, current channel: %d", bi->chanspec, notif_bss_info->channel));
		/* HTC_WIFI_END */
		...
		if (wl_is_linkup(cfg, e, ndev)) {
			...
			if (!wl_get_drv_status(cfg, DISCONNECTING, ndev)) {
				WL_ERR(("wl_bss_connect_done succeeded with " MACDBG "\n",
					MAC2STRDBG((u8*)(&e->addr))));
#ifdef CUSTOMER_HW_ONE
				if ((err = wl_bss_connect_done(cfg,
					ndev, e, data, true)) != 0) {
#ifdef REPORT_ROAM_DONE_EARLY
					if (err == AS_ROAM_DONE) {
						WL_ERR(("call roaming done instead\n"));
						err = wl_bss_roaming_done(cfg, ndev, e, data);
						if (unlikely(err)) {
							scb_val_t scbval;
							s32 reason = WLAN_REASON_DEAUTH_LEAVING;
							u8 *curbssid = wl_read_prof(cfg, ndev, WL_PROF_BSSID);

							WL_ERR(("%s roaming_done report err %d\n",
								__FUNCTION__, err));
							...
							wl_init_prof(cfg, ndev);
							sta_connected = 0;
							/* notify monitor to release perf lock when disconnected */
							wl_android_traffic_monitor(ndev);
							goto exit;
						}
					} else
#endif /* REPORT_ROAM_DONE_EARLY */
					{
						WL_ERR(("%s connection problem, err %d\n",
							__FUNCTION__, err));
			...
			sta_connected = 1;
#ifdef DHD_USE_IDLEACTIVE
			dhd_net_bus_idleclock(ndev, DHD_IDLE_ACTIVE);
#endif /* DHD_USE_IDLEACTIVE */
#endif /* CUSTOMER_HW_ONE */
	...
	const wl_event_msg_t *e, void *data)
{
...
#ifdef REPORT_ROAM_DONE_EARLY
	if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
		if (memcmp(curbssid, (void *)(e->addr.octet), ETHER_ADDR_LEN) == 0) {
			skip = true;
		}
	}
#endif /* REPORT_ROAM_DONE_EARLY */
	...
	if (unlikely(err)) {
		WL_ERR(("Could not get assoc ies %d\n", err));
		goto exit;
	}
	...
	err = wl_update_bss_info(cfg, ndev, true);
	if (unlikely(err)) {
		WL_ERR(("Could not update bss info %d\n", err));
		goto exit;
	}

	err = wl_update_pmklist(ndev, cfg->pmk_list, err);
	if (unlikely(err)) {
		WL_ERR(("Could not update pmklist %d\n", err));
		goto exit;
	}
#else
	wl_get_assoc_ies(cfg, ndev);
...
#ifdef REPORT_ROAM_DONE_EARLY
	if (skip) {
		WL_ERR(("roam reported, ignore it\n"));
		return err;
	}
#endif /* REPORT_ROAM_DONE_EARLY */
	...
	const wl_event_msg_t *e, void *data, bool completed)
{
	...
	if (wl_get_drv_status(cfg, CONNECTING, ndev)) {
		...
		if (completed) {
			...
			curbssid = wl_read_prof(cfg, ndev, WL_PROF_BSSID);
#ifdef CUSTOMER_HW_ONE
			if ((err = wl_update_bss_info(cfg, ndev, false)) != 0) {
				WL_ERR(("%s update bss info fail, skip the connection \n",
					__FUNCTION__));
				goto exit;
			}
#else
		...
		WL_ERR(("Don't report connect result - not connecting\n"));
#ifdef REPORT_ROAM_DONE_EARLY
		err = AS_ROAM_DONE;
#endif /* REPORT_ROAM_DONE_EARLY */
	}
...
#if defined(CUSTOMER_HW_ONE) && defined(HTC_TX_TRACKING)
	cfg->evt_handler[WLC_E_TX_STAT_ERROR] = wl_notify_txfail;
#endif
}
...
#ifdef CUSTOMER_HW_ONE
			if (!bi->RSSI) {
				WL_ERR(("%s("MACDBG"), bi  RSSI(%d) ZERO not update\n",
					bi->SSID, MAC2STRDBG(bi->BSSID.octet), bi->RSSI));
				goto exit;
			}
#endif /* CUSTOMER_HW_ONE */
	...
	enum wl_status state, bool set)
{
	...
	if (set) {
		...
		if (!cfg->vsdb_mode) {
			...
			/* Set ampdu ba wsize back to CUSTOM_AMPDU_BA_WSIZE */
...
#ifdef CUSTOM_WL_EVT_PRIO_SETTING
	wl_evt_prio = CUSTOM_WL_EVT_PRIO_SETTING;
#endif
	if (wl_evt_prio > 0) {
		struct sched_param param;
		param.sched_priority = wl_evt_prio;
		setScheduler(current, SCHED_FIFO, &param);
	}

	WL_ERR(("tsk Enter, tsk = 0x%p, wl_evt_prio=%d\n", tsk, wl_evt_prio));
		...
		while ((e = wl_deq_event(cfg))) {
			...
			if (e->etype < WLC_E_LAST && cfg->evt_handler[e->etype]) {
				...
				if (dhd->busstate == DHD_BUS_DOWN) {
					WL_ERR(("Event (%d) when BUS is DOWN.\n", e->etype));
				} else
	...
	u8 *peer, enum nl80211_tdls_operation oper)
{
	...
	/* HTC_WIFI_START */
	// ** Add log for debugging, make sure the tdls command pass to driver
	WL_ERR(("%s: nl80211_tdls_operation ==  %d\n", __FUNCTION__, oper));
	/* HTC_WIFI_END */
	if (peer) {
...
int wl_cfg80211_send_priv_event(struct net_device *dev, char *flag)
{
	...
	ret = wl_cfgvendor_send_async_event(cfg->wdev->wiphy,
		dev, BRCM_VENDOR_EVENT_PRIV_STR, flag, size);
...
void wl_cfg80211_abort_connecting(void)
{
	...
	if (!cfg) {
		WL_ERR(("%s : cfg is null, return\n", __func__));
		return;
	}

	dhd_pub = (dhd_pub_t *)(cfg->pub);

	dev = bcmcfg_to_prmry_ndev(cfg);
	if (!dev) {
		WL_ERR(("%s : dev is null, return\n", __func__));
		return;
	}
	if (wl_get_drv_status(cfg, CONNECTING, dev)) {
		int val = 0;
		int err;
		WL_ERR(("%s: abort the connecting procedure.\n", __FUNCTION__));
		...
		wl_bss_connect_done(cfg, dev, NULL, NULL, false);
	}
}
...
{
	WL_ERR(("deauth: %02X:%02X:%02X:%02X:%02x:%02X\n",
...
int wl_cfg80211_dhd_chk_link(void)
{
	s32 err;
	struct bcm_cfg80211 *cfg = NULL;
	struct net_device *dev = NULL;
...
// ** [HPKB#28650] Reduce log size on non-debug ROM
#if 0
	WL_ERR(("%s wl_get_drv_status(wl, CONNECTED, dev) = %d\n",
		__FUNCTION__, wl_get_drv_status(cfg, CONNECTED, dev)));
#else
	WL_ERR_HW_ONE(("%s wl_get_drv_status(wl, CONNECTED, dev) = %d\n",
		__FUNCTION__, wl_get_drv_status(cfg, CONNECTED, dev)));
#endif
/* HTC_WIFI_END */
