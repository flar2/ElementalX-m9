...
#define pr_fmt(fmt) "[CAM][FL]%s:%d " fmt, __func__, __LINE__
	...
	if(htc_torch_main && htc_flash_main)
	{
		if (flash_ctrl->pdev->id == 0)
		{
			...
			else
			{
				...
				pr_info("[CAM][FL] Main msm_flash_low, Set current value (50, 50)\n");
			}
		}
	}
	else
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	if(htc_flash_main && htc_torch_main)
	{
		if(flash_ctrl->pdev->id ==0)
		{
			...
			if(flash_data->flash_current[0] == -1 && flash_data->flash_current[1] == -1)
			{
				htc_flash_main(1000, 1000);
				pr_info("[CAM][FL] Main msm_flash_high, Set current value (1000, 1000)\n");
			}
			else
			{
	...
	void __user *argp)
{
	...
	case CFG_FLASH_RELEASE:
		//HTC_START, add protection
		if(flash_ctrl->func_tbl && flash_ctrl->func_tbl->camera_flash_release) {
		//HTC_END
            if (flash_ctrl->flash_state == MSM_CAMERA_FLASH_INIT)
			//HTC_START
			if (flash_ctrl->pdev->id == 0)
	...
	case CFG_FLASH_OFF:
		//HTC_START, add protection
		if(flash_ctrl->func_tbl && flash_ctrl->func_tbl->camera_flash_off) {
		//HTC_END
            if (flash_ctrl->flash_state == MSM_CAMERA_FLASH_INIT)
	...
	case CFG_FLASH_LOW:
		//HTC_START, add protection
		if(flash_ctrl->func_tbl && flash_ctrl->func_tbl->camera_flash_low) {
		//HTC_END
            if (flash_ctrl->flash_state == MSM_CAMERA_FLASH_INIT)
	...
	case CFG_FLASH_HIGH:
		//HTC_START, add protection
		if(flash_ctrl->func_tbl && flash_ctrl->func_tbl->camera_flash_high) {
		//HTC_END
            if (flash_ctrl->flash_state == MSM_CAMERA_FLASH_INIT)
