...
static uint32_t Module_Type = 0x00;    //0x00: In House module;   0x01: Altek module

//HTC_START, read OTP for dual cam calibration
#define DUAL_CAL_OTP_SIZE 2048    //total 32 pages, each page with 64 bytes
	...
	struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    if (first)
    {
        ...
        valid_layer = -1;
        for(page = 5; page >= 3; page--)   //page 3~5 for VCM info
        {
            rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client, 0x2A02, page, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
                pr_info("[CAM]%s: i2c_write w 0x2A02 fail\n", __func__);
        ...
        for(page = 10; page >= 8; page--)   //page 8~10 for Altek info
        {
            rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client, 0x2A02, page, MSM_CAMERA_I2C_BYTE_DATA);
            if (rc < 0)
                pr_info("[CAM]%s: i2c_write w 0x2A02 fail\n", __func__);
...
int32_t t4ka7_read_otp_memory(uint8_t *otpPtr, struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	//read full OTP memory: 32 pages, each page 64 bytes
...
int32_t t4ka7_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(rc != 0)
	{
		//HTC_CAM_START, workaround to probe t4ka7 old version
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
	...
	//HTC_CAM_START, workaround to avoid probe imx220
	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
