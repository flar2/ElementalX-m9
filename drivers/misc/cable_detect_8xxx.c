...
extern void si_wakeup_mhl(bool);
extern void si_suspend_mhl(void);
extern int mhl_detect_register_notifier(struct t_mhl_status_notifier *notifier);
#endif
...
struct cable_detect_info {
	...
	/*for accessory*/
	int usb_id_pin_gpio;
	int usb_id_connect2pm8994;
	u32 detect_type;
	__u8 accessory_type;
	uint32_t idpin_irq:16;
...
static DEFINE_MUTEX(usb_host_mhl_notify_sem);
void send_usb_host_mhl_connect_notify(int cable_in)
{
	struct t_usb_host_mhl_status_notifier *notifier;

	mutex_lock(&usb_host_mhl_notify_sem);
	list_for_each_entry(notifier,
		&g_lh_usb_host_mhl_detect_notifier_list,
		usb_host_mhl_notifier_link) {
		if (notifier->func != NULL) {
			CABLE_INFO("[HostMHLNotify] Send to: %s: %d\n",
	...
	mutex_unlock(&usb_host_mhl_notify_sem);
}

int usb_host_mhl_detect_register_notifier(struct t_usb_host_mhl_status_notifier *notifier)
{
	if (!notifier || !notifier->name || !notifier->func)
		return -EINVAL;

	mutex_lock(&usb_host_mhl_notify_sem);
	list_add(&notifier->usb_host_mhl_notifier_link,
			&g_lh_usb_host_mhl_detect_notifier_list);
	mutex_unlock(&usb_host_mhl_notify_sem);
	return 0;
}
...
void release_audio_dock_lock(void)
{
	...
	/* trigger irq to detect current status immediately */
...
/*-- 2014/11/12, USB Team, PCN00040 --*/
	...
	case DOCK_STATE_MHL:
		...
		usb_set_connect_type(CONNECT_TYPE_MHL_UNKNOWN);
/*++ 2014/12/19 USB Team, PCN00060 ++*/
		send_usb_host_mhl_connect_notify(CABLE_TYPE_MHL);
/*-- 2014/12/19 USB Team, PCN00060 --*/
#ifdef CONFIG_HTC_MHL_DETECTION_8620
		msleep(50);
		pr_info("pInfo->mhl_internal_3v3: %d, vbus: %d", pInfo->mhl_internal_3v3, vbus);
		si_wakeup_mhl(!pInfo->mhl_internal_3v3 && !vbus);
#endif
		break;
	case DOCK_STATE_USB_HOST:
		CABLE_INFO("USB Host inserted\n");
/*++ 2014/12/19 USB Team, PCN00060 ++*/
		send_usb_host_mhl_connect_notify(CABLE_TYPE_HOST);
/*-- 2014/12/19 USB Team, PCN00060 --*/
	...
	case DOCK_STATE_UNDOCKED:
		...
		case DOCK_STATE_MHL:
			...
			si_suspend_mhl();
			pInfo->accessory_type = DOCK_STATE_UNDOCKED;
			usb_set_connect_type(CONNECT_TYPE_NONE);
/*++ 2014/12/19 USB Team, PCN00060 ++*/
			send_usb_host_mhl_connect_notify(CABLE_TYPE_NONE);
/*-- 2014/12/19 USB Team, PCN00060 --*/
			break;
		case DOCK_STATE_USB_HOST:
...
static int64_t htc_get_usbid_adc(void)
{
	struct cable_detect_info *pInfo = &the_cable_info;
	...
	if (!pInfo->vadc_chip) {
		/*FIXME: Add if-condition for XA01/XA02 */
		if (pInfo->usb_id_connect2pm8994 == 1)
			pInfo->vadc_chip = qpnp_get_vadc(pInfo->dev, "usb_id_xa");
		else
			pInfo->vadc_chip = qpnp_get_vadc(pInfo->dev, "usb_id");
		if (IS_ERR(pInfo->vadc_chip)) {
			err = PTR_ERR(pInfo->vadc_chip);
			pInfo->vadc_chip = NULL;
			return -EAGAIN;
		}
	}

	err = qpnp_vadc_read(pInfo->vadc_chip, pInfo->adc_channel, &result);
	if (err < 0) {
		CABLE_INFO("%s: Read ADC fail, ret = %d\n", __func__, err);
		return err;
	}

	adc = result.physical / 1000;
	return adc;
}
...
static int cable_detect_get_adc(void)
{
	struct cable_detect_info *pInfo = &the_cable_info;
	long val;
	if (pInfo->get_adc_cb == NULL) {
		while((val = htc_get_usbid_adc()) == -EAGAIN)
		{
...
static int second_detect(struct cable_detect_info *pInfo)
{
	...
	/* FIXME: short term enable the adc range for MHL detect */
	/*if ((pInfo->mhl_version_ctrl_flag) || (adc_value >= 776 && adc_value <= 1020))*/
	if ((pInfo->mhl_version_ctrl_flag) || (adc_value >= 480 && adc_value <= 1020))
#ifdef CONFIG_HTC_MHL_DETECTION
		type = DOCK_STATE_MHL;
#else
		type = DOCK_STATE_MHL;
...
static void usb_id_detect_init(struct cable_detect_info *pInfo)
{
	...
	CABLE_INFO("%s: request_irq ret = %d, idpin_irq = %d\n", __func__,ret,pInfo->idpin_irq);
	if (ret < 0) {
...
#ifdef CONFIG_HTC_MHL_DETECTION_8620
static void mhl_8620_status_notifier_func(int charging_type)
{
	CABLE_INFO("%s: charging type %d\n",
				__func__, charging_type);
	switch (charging_type) {
		case 100:
			usb_set_connect_type(CONNECT_TYPE_MHL_100MA);
			break;
		case 500:
			usb_set_connect_type(CONNECT_TYPE_MHL_500MA);
			break;
		case 900:
			usb_set_connect_type(CONNECT_TYPE_MHL_900MA);
			break;
		case 1500:
			usb_set_connect_type(CONNECT_TYPE_MHL_1500MA);
			break;
		case 2000:
			usb_set_connect_type(CONNECT_TYPE_MHL_2000MA);
			break;
		default:
			usb_set_connect_type(CONNECT_TYPE_MHL_UNKNOWN);
			break;
...
static struct t_mhl_status_notifier mhl_8620_status_notifier = {
	.name = "mhl_detect",
	.func = mhl_8620_status_notifier_func,
};
#endif
...
#ifdef CONFIG_HTC_MHL_DETECTION_8620
void mhl_remove_notify_cable_detect(void)
{
	struct cable_detect_info *pInfo = &the_cable_info;
	CABLE_INFO("mhl_remove_notify_cable_detect: MHL removed\n");
	if(pInfo)
...
static int cable_detect_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		of_property_read_u32(node, "usb-id-connect2pm8994", &(pInfo->usb_id_connect2pm8994));
		printk("[CABLE] %s: usb-id-connect2pm8994=%d\n",__func__, pInfo->usb_id_connect2pm8994);
		if (pInfo->usb_id_connect2pm8994 == 1) {
			/*FIXME: Add if-condition for XA01/XA02 */
			pInfo->usb_id_pin_gpio = of_get_named_gpio(node, "htc,usb-id-gpio-xa", 0);
			of_property_read_u32(node, "usb-id-adc-channel-xa", &(pInfo->adc_channel));
		} else {
			pInfo->usb_id_pin_gpio = of_get_named_gpio(node, "htc,usb-id-gpio", 0);
			of_property_read_u32(node, "usb-id-adc-channel", &(pInfo->adc_channel));
		}
		of_property_read_u32(node, "detect-type", &(pInfo->detect_type));
		of_property_read_u32(node, "usb-id-pin-type", &usb_id_pin_type);

		printk(KERN_WARNING "[CABLE] %s(%d):id_gpio=%d, detect_type=%d, usb_id_pin_type=%d, adc_channel=%d\n",
			__func__, __LINE__, pInfo->usb_id_pin_gpio,
			pInfo->detect_type, usb_id_pin_type, pInfo->adc_channel);
...
#endif
#if (defined(CONFIG_CABLE_DETECT_ACCESSORY) && defined(CONFIG_HTC_MHL_DETECTION_8620))
	mhl_detect_register_notifier(&mhl_8620_status_notifier);
#endif
...
void usb_set_connect_type(int _connect_type)
{
	...
	if (pInfo->accessory_type == DOCK_STATE_MHL
		&& _connect_type != CONNECT_TYPE_MHL_UNKNOWN
		&& _connect_type != CONNECT_TYPE_MHL_100MA
		&& _connect_type != CONNECT_TYPE_MHL_500MA
		&& _connect_type != CONNECT_TYPE_MHL_900MA
		&& _connect_type != CONNECT_TYPE_MHL_1500MA
		&& _connect_type != CONNECT_TYPE_MHL_2000MA)
		_connect_type = pInfo->connect_type;
...
void usb_status_notifier_func(int cable_type)
{
	...
	if (pInfo->connect_type >= CONNECT_TYPE_MHL_AC || pInfo->connect_type == CONNECT_TYPE_MHL_UNKNOWN) {
		CABLE_INFO("%s: ignore cable type when connected to MHL dongle \n", __func__);
		return;
	}

	/* Set cable type to notify until received PC suspend/reset signal */
	if (cable_type == CONNECT_TYPE_USB)
		cable_type = CONNECT_TYPE_NOTIFY;
...
#endif
	pInfo->connect_type = cable_type;
	/* cable detect driver won't notify anybody*/
	/*send_cable_connect_notify(cable_type);*/

}
EXPORT_SYMBOL(usb_status_notifier_func);
...
static struct t_cable_status_notifier __maybe_unused usb_status_notifier = {
	.name = "cable_detect",
