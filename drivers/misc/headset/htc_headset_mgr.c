/*
 *
 * /driver/misc/headset/htc_headset_mgr.c
 *
...
static int headset_gpio_init(struct htc_hs_mgr_data *mgr);
//static int uart_rxtx_gpo_config(struct htc_hs_mgr_data *mgr, int mode);
...
static void headset_power(int enable)
{
	...
	gpio_direction_output(htc_headset_mgr_data.bias_gpio, enable);
}
...
static int headset_gpio_init(struct htc_hs_mgr_data *mgr)
{
	...
	set_state =
		pinctrl_lookup_state(mgr->hs_pinctrl,
					"headset_gpios");
	if (IS_ERR(set_state)) {
		HS_LOG("cannot get hs pinctrl gpio state\n");
		return PTR_ERR(set_state);
	}

	retval = pinctrl_select_state(mgr->hs_pinctrl, set_state);
	if (retval) {
		HS_LOG("cannot set hs pinctrl gpio state\n");
		return retval;
	}
	if (hi->pdata.uart_lv_shift_en)
		hi->pdata.uart_lv_shift_en(1);
	set_35mm_hw_state(0);
	HS_LOG("headset_gpio_init done\n");
	return 0;
	...
	if (mode) {
		...
		set_state =
			pinctrl_lookup_state(mgr->hs_pinctrl,
						"hs_txrx_pin_alt");
		if (IS_ERR(set_state)) {
			HS_LOG("cannot get hs pinctrl alt state\n");
			return PTR_ERR(set_state);
		}
	}
	retval = pinctrl_select_state(mgr->hs_pinctrl, set_state);
	if (retval) {
		HS_LOG("cannot set hs pinctrl gpio_alt state\n");
		return retval;
	}
...
{
	int OEz = 0;
	if (htc_headset_mgr_data.oe_gpio) {
		gpio_direction_output(htc_headset_mgr_data.oe_gpio, enable);
...
static int headset_mgr_parser_dt(struct htc_hs_mgr_data *mgr)
{
	...
	for (i = 0; i < ARRAY_SIZE(parser_st); i++) {
		...
		if (!gpio_is_valid(gpio[i])) {
			HS_LOG("DT_mgr no %s parser, ret = %d",parser_st[i], gpio[i]);
			//goto parser_fail;
		}
	}
	...
	if (htc_headset_mgr_data.oe_gpio) {
		ret = gpio_request(htc_headset_mgr_data.oe_gpio, "headset_oe");
		if (ret < 0)
			HS_LOG("gpio_request headset_oe fail!\n");
		else {
			HS_LOG("gpio_request headset_oe OK!\n");
			gpio_direction_output(htc_headset_mgr_data.oe_gpio, 1);
		}
	}

	ret = gpio_request(htc_headset_mgr_data.bias_gpio, "headset_bias");
	if (ret < 0)
		HS_LOG("gpio_request headset_bias fail!\n");
	else {
		HS_LOG("gpio_request headset_bias OK!\n");
		gpio_direction_output(htc_headset_mgr_data.bias_gpio, 0);
	}

	ret = of_property_read_u32(dt, paser_alt, (u32*)&htc_headset_mgr_data.wire_alt);
	if (ret < 0) {		//alt func is never be 0. 0 is gpio func
...
static int htc_headset_mgr_probe(struct platform_device *pdev)
{
	...
	dtinfo->hs_pinctrl = devm_pinctrl_get(dtinfo->dev);
	if (IS_ERR(dtinfo->hs_pinctrl)) {
		if (PTR_ERR(dtinfo->hs_pinctrl) == -EPROBE_DEFER)
			return -EPROBE_DEFER;

		HS_LOG("Target does not use pinctrl\n");
		dtinfo->hs_pinctrl = NULL;
	}

	if (dtinfo->hs_pinctrl) {
		ret = headset_gpio_init(dtinfo);
		if (ret) {
			HS_ERR("cannot set ts pinctrl active state\n");
...
device_initcall(htc_headset_mgr_init);
