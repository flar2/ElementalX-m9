...
#ifdef GPIO_POWER
static int fpc_power_control(int power_state, struct fpc1020_platform_data *pdata);
#endif
...
static int fpc1020_probe(struct spi_device *spi)
{
...
#ifndef DO_CHIP_SELECT
	struct pinctrl *cs2_pinctrl;
...
#ifdef GPIO_POWER
    error = fpc_power_control(FPC1020_POWER_STATE_ON,fpc1020_pdata);
#else
	...
	cs2_pinctrl = devm_pinctrl_get(&spi->dev);
	if (IS_ERR(cs2_pinctrl)) {
			if (PTR_ERR(cs2_pinctrl) == -EPROBE_DEFER)
					return -EPROBE_DEFER;
			printk("[fp][CT]Target does not use pinctrl\n");
			cs2_pinctrl = NULL;
	}
	set_state = pinctrl_lookup_state(cs2_pinctrl,"spi_0_cs2_active");
	if (IS_ERR(set_state)) {
			printk("[fp][CT]Cannot get SPI-CS2 pinctrl gpio state\n");
			return PTR_ERR(set_state);
	}

	error = pinctrl_select_state(cs2_pinctrl, set_state);
	if (error) {
			printk("[fp][CT]Cannot set SPI-CS2 pinctrl gpio state\n");
			return error;
	}
...
err:
#ifdef GPIO_POWER
	if (gpio_is_valid(fpc1020_pdata->pwr_3v3_gpio))
		gpio_free(fpc1020_pdata->pwr_3v3_gpio);

	if (gpio_is_valid(fpc1020_pdata->pwr_1v8_gpio))
		gpio_free(fpc1020_pdata->pwr_1v8_gpio);
#endif
					...
					struct fpc1020_platform_data *pdata)
{
	...
	fpc1020->spi->max_speed_hz = 9600000;
...
static int fpc_power_control(int power_state, struct fpc1020_platform_data *pdata)
{
	...
	if (retval) {
		printk("[fp][CT] gpio_request (1v8) failed.\n");
		return retval;
	}

	retval = gpio_request(pdata->pwr_3v3_gpio, "fpc_3v3");
	if (retval) {
		printk("[fp][CT] gpio_request (3v3) failed.\n");
		return retval;
	}


	if(power_state == FPC1020_POWER_STATE_ON)
	{
		retval = gpio_direction_output(pdata->pwr_1v8_gpio, 0);
		if (retval) {
			printk("[fp][CT] gpio_direction_output (1v8) failed.\n");
			return retval;
		}
		...
		if (retval) {
			printk("[fp][CT] gpio_direction_output (3v3) failed.\n");
			return retval;
		}
	...
	} else if(power_state == FPC1020_POWER_STATE_OFF) {
        printk("[fp][CT] Power off \n");
    	if (gpio_is_valid(pdata->pwr_3v3_gpio)) {
			gpio_set_value(pdata->pwr_3v3_gpio, 0);
			gpio_free(pdata->pwr_3v3_gpio);
    	}

    	if (gpio_is_valid(pdata->pwr_1v8_gpio)) {
            gpio_set_value(pdata->pwr_1v8_gpio, 0);
			gpio_free(pdata->pwr_1v8_gpio);
    	}
	}
