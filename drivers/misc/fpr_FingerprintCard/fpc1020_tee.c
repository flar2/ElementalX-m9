...
static int spi_set_fabric(struct fpc1020_data *fpc1020, bool active)
{
    ...
    dev_info(fpc1020->dev, "%s:%d rc:%d cnt:%d\n", __func__, active, rc, cnt);
    return rc;
}
...
static int set_pipe_ownership(struct fpc1020_data *fpc1020, bool to_tz)
{
    ...
    dev_info(fpc1020->dev, "%s: Set_Pipe_ownerShip+\n", __func__);
...
{
	int rc = 0;
	dev_info(fpc1020->dev, "%s: enable:%d clocks_enabled:%d\n", __func__, enable, fpc1020->clocks_enabled);
...
//		dev_info(fpc1020->dev, "%s enable ok. clk rate %u hz\n", __func__,
//				fpc1020->spi->max_speed_hz);
		...
		dev_info(fpc1020->dev, "%s disable ok\n", __func__);
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if ((*buf == '1')) {
        ...
        rc = spi_set_fabric(fpc1020, false);
        rc = select_pin_ctl(fpc1020, "fpc1020_spi_suspend");
    }
...
static int device_prepare(struct  fpc1020_data *fpc1020, bool enable)
{
    ...
    if (enable && !fpc1020->prepared) {
        dev_info(dev, "%s(%d): fpc1020_spi_setup enable-a:%d\n", __func__, __LINE__, enable);
        spi_bus_lock(fpc1020->spi->master);
        ...
        dev_info(dev, "%s(%d): fpc1020_spi_setup enable-b:%d\n", __func__, __LINE__, enable);
        rc = 0;
...
exit:
        ...
        dev_info(dev, "%s(%d): fpc1020_spi_setup enable-c:%d\n", __func__, __LINE__, enable);
        rc = 0;
    }
    mutex_unlock(&fpc1020->lock);
    return rc;
...
{
    struct fpc1020_data *fpc1020 = dev_get_drvdata(device);
    printk("fpc fpc1020->fp_source = %d\n", fpc1020->fp_source);
    return scnprintf(buffer, PAGE_SIZE, "%i\n", fpc1020->fp_source);
}
...
static int id_pin_detect(struct fpc1020_data *fp){
	...
	dev_dbg(dev, "fpc %s +++\n", __func__);
	fp->fp_source = 1;		// Default : Main source : fpc : 1

	rc = select_pin_ctl(fp, "fpc1020_id_init"); // id_pin I/PD for init state
	if(rc){
		dev_err(dev, "fpc %s, select_pin_ctrl(fp, fpc1020_id_init) failed !", __func__);
		goto error;
	}

	rc = select_pin_ctl(fp, "fpc1020_id_active"); // id_pin I/PU for active state
 	if(rc){
 		dev_err(dev, "fpc %s, select_pin_ctrl(fp, fpc1020_id_active) failed !", __func__);
 		goto error;
 	}
	id_pin = gpio_get_value(fp->id_gpio);
	dev_dbg(dev,"[FP] detect id pin = %s\n", (id_pin? "High" : "Low"));

	if((id_pin == 1)){
		fp->fp_source = 1;
		dev_dbg(dev, "fpc Fingerprint source = FPC\n");
	}
	else if((id_pin == 0)){
		fp->fp_source = 0;
		dev_dbg(dev, "fpc Fingerprint source = IDEX\n");
        }
        else{
	...
	dev_dbg(dev, "fpc %s ---\n", __func__);
	rc = select_pin_ctl(fp, "fpc1020_id_sleep"); // id_pin I/PD for sleep state
	if(rc){
...
#ifdef CONFIG_HTC_ID_PIN
        rc = fpc1020_request_named_gpio(fpc1020, "fpc_gpio_id",
			&fpc1020->id_gpio);
	if (rc)
		goto exit;
#endif //CONFIG_HTC_ID_PIN
...
//		goto exit;
	...
	if (of_property_read_bool(dev->of_node, "fpc,enable-on-boot")) {
		dev_info(dev, "Enabling hardware start++\n");
		(void)device_prepare(fpc1020, true);
		dev_info(dev, "Enabling hardware done++\n");
