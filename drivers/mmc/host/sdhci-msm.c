...
struct workqueue_struct *dump_snoc_mmc_wq;
static void dump_snoc_process_mmc(struct work_struct *work);
static DECLARE_WORK(dump_snoc_work_mmc, dump_snoc_process_mmc);
...
void sdhci_msm_reset_workaround(struct sdhci_host *host, u32 enable)
{
	...
	queue_work(dump_snoc_mmc_wq, &dump_snoc_work_mmc);
}
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	dump_snoc_mmc_wq = create_singlethread_workqueue("dump_snoc_work_mmc");
	if (!dump_snoc_mmc_wq)
		pr_err("%s create workqueue dump_snoc_work fail\n",
			mmc_hostname(host->mmc));
...
static int sdhci_msm_remove(struct platform_device *pdev)
{
	...
	if (!dump_snoc_mmc_wq)
		destroy_workqueue(dump_snoc_mmc_wq);
	return 0;
}
