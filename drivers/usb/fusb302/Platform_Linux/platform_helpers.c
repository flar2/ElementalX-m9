...
/* HTC SSD USB + */
#define HTC_DT_GPIO_CC_OVP		"htc,cc_ovp"
#define HTC_DT_GPIO_AUO_CONTROL		"htc,aud_control"
#define HTC_DT_GPIO_IN1			"htc,ncn1188_in1"
#define HTC_DT_GPIO_IN2			"htc,ncn1188_in2"
#define HTC_DT_GPIO_VCONN		"fairchild,vconn-gpio"
...
FSC_S32 fusb_InitializeGPIO(void)
{
    ...
    struct pinctrl_state *set_state;  /* HTC SSD USB */
    ...
    if (chip->fusb302_pinctrl) {
	    set_state = pinctrl_lookup_state(chip->fusb302_pinctrl, "fusb_typec-gpio"); //Int_N
	    if (IS_ERR(set_state)) {
		    pr_err("FUSB: cannot get fusb302 pinctrl default state");
		    return PTR_ERR(set_state);
	    }
	    ret = pinctrl_select_state(chip->fusb302_pinctrl, set_state);
	    if (ret) {
		    dev_err(&chip->client->dev, "FUSB  %s - Error: pinctrl enable Int_N state failed:%d\n", __func__, ret);
		    return ret;
	    }

	    set_state = pinctrl_lookup_state(chip->fusb302_pinctrl, "fusb_typec-gpio2"); //Vconn
	    if (IS_ERR(set_state)) {
		    pr_err("FUSB: cannot get fusb302 pinctrl vconn default state");
		    return PTR_ERR(set_state);
	    }
	    ret = pinctrl_select_state(chip->fusb302_pinctrl, set_state);
	    if (ret) {
		    dev_err(&chip->client->dev, "FUSB  %s - Error: pinctrl enable vconn state failed:%d\n", __func__, ret);
		    return ret;
	    }

	    set_state = pinctrl_lookup_state(chip->fusb302_pinctrl, "fusb_typec-gpio3"); //CC ovp
	    if (IS_ERR(set_state)) {
		    pr_err("FUSB: cannot get fusb302 pinctrl cc ovp default state");
		    return PTR_ERR(set_state);
	    }
	    ret = pinctrl_select_state(chip->fusb302_pinctrl, set_state);
	    if (ret) {
		    dev_err(&chip->client->dev, "FUSB  %s - Error: pinctrl enable cc ovp state failed:%d\n", __func__, ret);
		    return ret;
	    }

	    set_state = pinctrl_lookup_state(chip->fusb302_pinctrl, "fusb_typec-gpio4"); //audio control
	    if (IS_ERR(set_state)) {
		    pr_err("FUSB: cannot get fusb302 pinctrl audio control default state");
		    return PTR_ERR(set_state);
	    }
	    ret = pinctrl_select_state(chip->fusb302_pinctrl, set_state);
	    if (ret) {
		    dev_err(&chip->client->dev, "FUSB  %s - Error: pinctrl enable audio control state failed:%d\n", __func__, ret);
		    return ret;
	    }
    }

    chip->gpio_cc_ovp = of_get_named_gpio(node, HTC_DT_GPIO_CC_OVP, 0);
    if (!gpio_is_valid(chip->gpio_cc_ovp)) {
	    dev_err(&chip->client->dev, "%s - Error: Could not get named GPIO for CC OVP! Error code: %d\n", __func__, chip->gpio_cc_ovp);
	    return chip->gpio_cc_ovp;
    }

    ret = gpio_request(chip->gpio_cc_ovp, HTC_DT_GPIO_CC_OVP);
    if (ret < 0) {
	    dev_err(&chip->client->dev, "%s - Error: Could not request GPIO for CC OVP! Error code: %d\n", __func__, ret);
	    return ret;
    }

    ret = gpio_direction_input(chip->gpio_cc_ovp);
    if (ret < 0) {
	    dev_err(&chip->client->dev, "%s - Error: Could not set GPIO direction to input for CC OVP! Error code: %d\n", __func__, ret);
	    return ret;
    }

    pr_info("FUSB  %s - CC OVP GPIO initialized as pin '%d'\n", __func__, chip->gpio_cc_ovp);

    chip->gpio_aud_control = of_get_named_gpio(node, HTC_DT_GPIO_AUO_CONTROL, 0);
    if (!gpio_is_valid(chip->gpio_aud_control)) {
	    dev_err(&chip->client->dev, "%s - Error: Could not get named GPIO for Audio control! Error code: %d\n", __func__, chip->gpio_aud_control);
	    return chip->gpio_aud_control;
    }
    ret = gpio_request(chip->gpio_aud_control, HTC_DT_GPIO_AUO_CONTROL);
    if (ret < 0) {
	    dev_err(&chip->client->dev, "%s - Error: Could not request GPIO for Audio control! Error code: %d\n", __func__, ret);
	    return ret;
    }

    pr_info("FUSB  %s - Audio control GPIO initialized as pin '%d'\n", __func__, chip->gpio_aud_control);

    chip->gpio_in1 = of_get_named_gpio(node, HTC_DT_GPIO_IN1, 0);
    if (!gpio_is_valid(chip->gpio_in1)) {
	    dev_err(&chip->client->dev, "%s - Error: Could not get named GPIO for ncn1188 in1! Error code: %d\n", __func__, chip->gpio_in1);
	    return chip->gpio_in1;
    }
    ret = gpio_request(chip->gpio_in1, HTC_DT_GPIO_IN1);
    if (ret < 0) {
	    dev_err(&chip->client->dev, "%s - Error: Could not request GPIO for ncn1188 in1! Error code: %d\n", __func__, ret);
	    return ret;
    }

    chip->gpio_in2 = of_get_named_gpio(node, HTC_DT_GPIO_IN2, 0);
    if (!gpio_is_valid(chip->gpio_in2)) {
	    dev_err(&chip->client->dev, "%s - Error: Could not get named GPIO for ncn1188 in2! Error code: %d\n", __func__, chip->gpio_in2);
	    return chip->gpio_in2;
    }
    ret = gpio_request(chip->gpio_in2, HTC_DT_GPIO_IN2);
    if (ret < 0) {
	    dev_err(&chip->client->dev, "%s - Error: Could not request GPIO for ncn1188 in2! Error code: %d\n", __func__, ret);
	    return ret;
    }

    chip->gpio_vconn = of_get_named_gpio(node, HTC_DT_GPIO_VCONN, 0);
    if (!gpio_is_valid(chip->gpio_vconn)) {
	    dev_err(&chip->client->dev, "%s - Error: Could not get named GPIO for VCONN! Error code: %d\n", __func__, chip->gpio_vconn);
	    return chip->gpio_vconn;
    }
    ret = gpio_request(chip->gpio_vconn, HTC_DT_GPIO_VCONN);
    if (ret < 0) {
	    dev_err(&chip->client->dev, "%s - Error: Could not request GPIO for VCONN! Error code: %d\n", __func__, ret);
	    return ret;
    }
    /* HTC SSD USB - */
...
#else
    return false;//We don't support Vbus 5V, return false
#endif
}
...
#ifdef VBUS_OTHER_SUPPORTE
    return chip->gpio_VBusOther_value;
#else
    return false;//We don't support Vbus Other, return false
#endif
}
...
FSC_BOOL fusb_Power_Vconn(FSC_BOOL set)
{
	...
	if (set) {
		...
		gpio_set_value(chip->gpio_vconn, 1);
		msleep(20);
		pr_info("FUSB %s: the value of gpio_vconn:%d\n", __func__, gpio_get_value(chip->gpio_vconn));
	} else {
		if (regulator_disable(chip->boost_5v)) {
			pr_err("%s: Unable to disable boost_5v regulator\n", __func__);
			return FALSE;
		}
		gpio_set_value(chip->gpio_vconn, 0);
		msleep(20);
		pr_info("FUSB %s: the value of gpio_vconn:%d\n", __func__, gpio_get_value(chip->gpio_vconn));
	}
...
FSC_BOOL fusb_I2C_WriteData(FSC_U8 address, FSC_U8 length, FSC_U8* data)
{
	...
	if (atomic_read(&chip->pm_suspended)) {
		pr_err("FUSB %s: pm_suspended, retry\n", __func__);
    ...
    else if (chip->HostCommBuf[0] == CMD_READ_STATE_LOG)  // Parse out the Type-C state log
    {
        ...
        for (i = 4; (i + 4 < FSC_HOSTCOMM_BUFFER_SIZE) && (numChars < PAGE_SIZE) && (numLogs > 0); i += 5, numLogs--) // Must be able to peek 4 bytes ahead, and don't overflow the output buffer (PAGE_SIZE), only print logs we have
        {
            ...
            switch (chip->HostCommBuf[i])
            {
                ...
                {
                    numChars += sprintf(tempBuf, "[%u.%04u]\tDebugAccessorySource\n", TimeStampSeconds, TimeStampMS10ths);
                    strcat(buf, tempBuf);
                    break;
                }
		...
		{
			numChars += sprintf(tempBuf, "[%u.%04u]\tDebugAccessorySink\n", TimeStampSeconds, TimeStampMS10ths);
			strcat(buf, tempBuf);
			break;
		}
...
static DEVICE_ATTR(vconn_en, S_IWUSR, NULL, _fusb_Sysfs_vconn_en_store);
