...
/* HTC SSD USB + */
	if (sm_count == 0 || preConnState != ConnState)
		printk("FUSB %s: preConnState:%d, ConnState:%d, sm_count:%d\n", __func__, preConnState, ConnState, sm_count);
...
#endif // FSC_INTERRUPT_TRIGGERED
    printk("FUSB  [%s]: leave StateMachineTypeC(), now ConnState:%d, CCTermPrevious:%d, VConnTerm:%d, PortType:%d, blnSrcPreferred:%d, blnSnkPreferred:%d\n", __func__, ConnState, CCTermPrevious, VCONNTerm, PortType, blnSrcPreferred, blnSnkPreferred); 
	sm_count = 0;
	preConnState = 0;
}
...
{
    if((VCONNTerm >= CCTypeRdUSB) && (VCONNTerm <= CCTypeRd3p0) && (CCTermPrevious == CCTypeOpen))
    {
        ToggleMeasure();
        updateVCONNSink();
    }
    ...
    if ((CCTermPDDebounce == CCTypeOpen) && (VCONNTerm == CCTypeOpen)) // If we have detected SNK.Open for atleast tPDDebounce on both pins...
    {
...
#endif // FSC_HAVE_DRP
        {
            ...
            && (CCTermCCDebounce >= CCTypeRdUSB) && (CCTermCCDebounce < CCTypeUndefined) 
...
void StateMachineAttachWaitSource(void)
{
    ...
    if(blnAccSupport)                                                           // If accessory support is enabled
    {
        ...
        else if ((CCTermCCDebounce >= CCTypeRdUSB) && (CCTermCCDebounce < CCTypeUndefined) 
    ...
    else if (CCTermPrevious == CCTypeOpen)      // If our debounced signal is open, go to the unattached state
        SetStateDelayUnattached();
}
#endif // FSC_HAVE_SRC
...
#ifdef FSC_INTERRUPT_TRIGGERED
    else if((CCTermPrevious == CCTypeOpen) && (g_Idle == TRUE))
    {
        g_Idle = FALSE;                                                             // Mask comp - debounce CC as open
        Registers.Mask.M_COMP_CHNG = 1;
        DeviceWrite(regMask, 1, &Registers.Mask.byte);
	platform_notify_accessory(0);  /* HTC SSD USB + */
        platform_enable_timer(TRUE);
    }
    else if(CCTermPrevious == CCTypeRa)      // && because one Pin will always stay detected as Ra
    {
...
#endif /* FSC_HAVE_ACCMODE */
#ifdef FSC_HAVE_DRP
            if ((PortType == USBTypeC_DRP) && (CCTermPDDebounce == CCTypeOpen))
            {
...
#ifdef FSC_HAVE_ACCMODE    
    if (blnAccSupport && (CCTermPrevious == CCTypeRa) && (VCONNTerm == CCTypeRa))
    {
...
#ifdef FSC_HAVE_ACCMODE
void SetStateDebugAccessorySource(void) /* FUSB302(B) does not support Debug Source orientation */
{
    Registers.Mask.M_COMP_CHNG = 0;
    DeviceWrite(regMask, 1, &Registers.Mask.byte);
...
void SetStateAudioAccessory(void)
{
    ...
    platform_notify_accessory(1);  /* HTC SSD USB + */
