...
int himax_touch_fw_update_cap(struct firmware *vfw)
{
	...
	I("[FW] chip fw version = 0x%02X,%02X\n", ts->vendor_fw_ver_H, ts->vendor_fw_ver_L);
	//Check fw header
	if (vfw->data[0] == 'C' && vfw->data[1] == 'S') {
		...
		if (index != NULL) {
			...
			if (*(index + strlen(name)) == '_') {
				...
				I("[FW] image version: 0x%02X,%02X\n", file_ver_maj, file_ver_min);
...
void himax_HW_reset_cap(uint8_t loadconfig,uint8_t int_off)
{
	...
	//if (ts->rst_gpio) {
		...
		himax_rst_gpio_set_cap(ts->rst_gpio, 0);
		msleep(20);
		himax_rst_gpio_set_cap(ts->rst_gpio, 1);
		msleep(20);
...
static uint8_t vkey_previous = 0;
inline void himax_ts_work_cap(struct himax_ts_data *ts)
{
	...
	const char *vkey_name[] = {"KEY_BACK", "KEY_APPSELECT"};
...
bypass_checksum_failed_packet:
	...
	if (buf[HX_TOUCH_INFO_POINT_CNT] == 0xff
		...
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			...
			if ((vkey_previous ^ vkey_status) & BIT(loop_i))
				input_report_key(ts->input_dev,	vkey_code[loop_i], 0);
			input_sync(ts->input_dev);
		}
		...
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			...
			if ((vkey_previous ^ vkey_status) & BIT(loop_i))
				I("%s %s.\n", vkey_name[loop_i], "Released");
		}

		if ((ts->debug_log_level & BIT(1)) > 0) {
			I("{PN, ID Inf*2, ID Inf*3}={0x%02X, 0x%02X, 0x%02X}; vkey_previous=%d, vkey_status=%d\n",
		...
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			/* Check which key status has been changed, then report this change event */
			if ((vkey_previous ^ vkey_status) & BIT(loop_i))
				input_report_key(ts->input_dev,	vkey_code[loop_i], vkey_status & BIT(loop_i));
			input_sync(ts->input_dev);
		}
		...
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			/* Check which key status has been changed, then print this change */
			if ((vkey_previous ^ vkey_status) & BIT(loop_i))
				I("%s %s.\n", vkey_name[loop_i], (vkey_status & BIT(loop_i)) ? "Pressed" : "Released");
		}
				...
				struct himax_i2c_platform_data *pdata)
{
	...
	I(" DT:gpio_irq=%d, gpio_rst=%d, gpio_3v3_en=%d, gpio_1v8_en=%d", pdata->gpio_irq, pdata->gpio_reset, pdata->gpio_3v3_en, pdata->gpio_1v8_en);
...
/*
	gpio_state_suspend
		= pinctrl_lookup_state(ts_pinctrl, "pmx_ts_power_off");
	if (IS_ERR_OR_NULL(gpio_state_suspend)) {
		I("Can not get ts sleep pinstate\n");
		ret = PTR_ERR(gpio_state_suspend);
		ts_pinctrl = NULL;
		return ret;
	}
*/
...
static void himax_pinctrl_deinit(struct device *dev)
{
	...
	if (ts_pinctrl != NULL) {
		...
		gpio_reset_active = NULL;
		gpio_reset_suspend = NULL;
...
static int himax852xes_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//IN ACA, temp, do not load config Himax Power On and Load Config
...
wake_lock_init(&ts->ts_flash_wake_lock, WAKE_LOCK_SUSPEND, HIMAX852xes_NAME_CAP);
	...
	if (private_ts->update_feature){
		if ((strcmp(mfg_mode, "")  == 0) ||
			...
			kthread_run(updateFirmware, (void *)ts, "HMX_FW_UPDATE_CAP");
		} else
