...
/* HTC_TP_START, HTC_TP_SRC_CTL */
static int g_tw_sensor_id = -1;
/* HTC_TP_END, HTC_TP_SRC_CTL */
...
DECLARE_COMPLETION(fwu_remove_complete_v2_6);
DEFINE_MUTEX(fwu_sysfs_mutex_v2_6);
...
/* HTC_TP_START */
	retval = 0;
	memset(rmi4_data->config_id, 0, CONFIG_ID_STRING_SIZE);
	for (i = 0; i < CONFIG_ID_STRING_SIZE; i++)
		retval += snprintf(rmi4_data->config_id + retval, CONFIG_ID_STRING_SIZE - retval, "%02X", fwu->config_id[i]);
/* HTC_TP_END */
...
/* HTC_TP_START */
	char image_config_id[80] = {0};
	char *str1 = image_config_id;
/* HTC_TP_END */
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
...
/* HTC_TP_START */
		snprintf(str1, 3, "%02X", fwu->img.ui_config.data[ii]);
		str1 += 2;
...
exit:
/* HTC_TP_START */
	dev_info(rmi4_data->pdev->dev.parent, "%s: Device config ID '%s'\n",
			__func__, image_config_id);
	dev_info(rmi4_data->pdev->dev.parent, "%s: Image config ID '%s'\n",
			__func__, rmi4_data->config_id);
/* HTC_TP_END */
	if (flash_area == NONE) {
		dev_info(rmi4_data->pdev->dev.parent,
...
/* HTC_TP_START, HTC_TP_SRC_CTL */
static int fwu_read_tw_sensor_id_v5v6(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	uint8_t data[6] = {0};
	uint16_t tw_pin_mask = rmi4_data->hw_if->board_data->tw_pin_mask;

	data[2] = data[0] = (unsigned char)(tw_pin_mask & 0x00FF);
	data[3] = data[1] = (unsigned char)((tw_pin_mask & 0xFF00) >> 8);
	...
	rmi4_data->tw_sensor_id = (data[5] << 8) | data[4];

	dev_info(rmi4_data->pdev->dev.parent, " %s: tw_sensor_id = 0x%x\n",
			__func__,
			rmi4_data->tw_sensor_id);

	return 0;
}

static int fwu_read_tw_sensor_id_v7(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	...
	data[6] = data[8] = (unsigned char)(tw_pin_mask & 0x00FF);
	data[7] = data[9] = (unsigned char)((tw_pin_mask & 0xFF00) >> 8);
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				fwu->f34_fd.data_base_addr + fwu->off.payload,
				data,
				sizeof(data));
...
static unsigned int fwu_get_sensor_id(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;

	if (fwu->bl_version == BL_V7)
		retval = fwu_read_tw_sensor_id_v7(rmi4_data);
	else
		retval = fwu_read_tw_sensor_id_v5v6(rmi4_data);

	if (retval < 0)
		return 0;

	g_tw_sensor_id = rmi4_data->tw_sensor_id;
	return rmi4_data->tw_sensor_id;
}

int htc_get_tw_sensor_id(void)
{
	return g_tw_sensor_id;
}
EXPORT_SYMBOL(htc_get_tw_sensor_id);

static unsigned int fwu_get_display_id(struct synaptics_rmi4_data *rmi4_data)
{
	unsigned int display_id = 0;
	return (display_id << 3);
}

static unsigned int fwu_get_dev_gpio_id(struct synaptics_rmi4_data *rmi4_data)
{
	unsigned int gpio_id = 0;
	return (gpio_id << 6);
}

static const char *fwu_get_tp_device(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	uint32_t i, condition;
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;

	retval = fwu_enter_flash_prog();
	if (retval >= 0)
		condition = fwu_get_sensor_id(rmi4_data) | fwu_get_display_id(rmi4_data) | fwu_get_dev_gpio_id(rmi4_data);
	else
		dev_err(rmi4_data->pdev->dev.parent,
				"Failed to enter BL, sensor id was not readed\n");

	rmi4_data->reset_device(rmi4_data, false);

	for (i = 0; i < TP_SRC_NUM; i++) {
		if (condition == bdata->tp_src_id[i]) {
			dev_info(rmi4_data->pdev->dev.parent,
					"TP FW: %s, id %#x\n", bdata->tp_src[i], condition);
			return bdata->tp_src[i];
		}
	}

	dev_err(rmi4_data->pdev->dev.parent, "TP FW: no matching FW for id %#x\n", condition);
	for (i = 0; i < TP_SRC_NUM; i++) {
		dev_err(rmi4_data->pdev->dev.parent,
				"id: %#x, src: %s:\n", bdata->tp_src_id[i], bdata->tp_src[i]);
	}
	return NULL;
}
...
/* HTC_TP_END, HTC_TP_SRC_CTL */
	...
	if (fwu->image == NULL) {
/* HTC_TP_START, HTC_TP_SRC_CTL */
		fw_image_name = fwu_get_tp_device(rmi4_data);
...
int synaptics_fw_updater_v2_6(const unsigned char *fw_data)
{
	int retval;
...
EXPORT_SYMBOL(synaptics_fw_updater_v2_6);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	synaptics_fw_updater_v2_6(NULL);
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (!mutex_trylock(&fwu_sysfs_mutex_v2_6))
		return -EBUSY;
...
show_image_exit:
	mutex_unlock(&fwu_sysfs_mutex_v2_6);
	return retval;
}
...
/* HTC_TP_END */
	...
	retval = synaptics_fw_updater_v2_6(fwu->image);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to do reflash\n",
				__func__);
...
exit:
	complete(&fwu_remove_complete_v2_6);
...
{
	synaptics_rmi4_new_function_v2_6(&fwu_module, true);
...
{
	synaptics_rmi4_new_function_v2_6(&fwu_module, false);

	wait_for_completion(&fwu_remove_complete_v2_6);
