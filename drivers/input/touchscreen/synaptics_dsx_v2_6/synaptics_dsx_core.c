...
/* HTC_TP_START, HTC_TP_PINCTRL */
#include <linux/pinctrl/consumer.h>
/* HTC_TP_END, HTC_TP_PINCTRL */
/* HTC_TP_START, HTC_TP_DMASK */
#include <trace/events/power.h>
/* HTC_TP_END, HTC_TP_DMASK */
...
/* HTC_TP_START, HTC_TP_ATTR */
#define F01_CHIP_ID_OFFSET 17
/* HTC_TP_END, HTC_TP_ATTR */
...
/* HTC_TP_END, HTC_TP_DMASK */
/* HTC_TP_START, HTC_TP_NOISE */
#define FREQ_MASK 0x7F
/* HTC_TP_END, HTC_TP_NOISE */
/* HTC_TP_START, HTC_TP_ATTR */
...
/* HTC_TP_END, HTC_TP_ATTR */
/* HTC_TP_START, HTC_TP_DMASK */
static uint32_t debug_mask = BIT(3);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (reset == 1) {
		...
		dev_info(rmi4_data->pdev->dev.parent, "%s: hw reset\n", __func__);
	} else {
		retval = synaptics_rmi4_reset_device(rmi4_data, false);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret += scnprintf(buf + ret, PAGE_SIZE, "_PR: %d\n", rmi4_data->firmware_id);
		...
		struct device_attribute *attr, char *buf)
{
	...
	case 3708:
	case 3718:
		config_id_size = 32/* V7_CONFIG_ID_SIZE */;
		break;
	default:
		config_id_size = 4/* V5V6_CONFIG_ID_SIZE */;
		break;
	}
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent," %s err %ld\n", __func__, retval);
		return retval;
	}

	ret += snprintf(buf + ret, PAGE_SIZE, "  config_id: ");

	for (ii = 0; ii < config_id_size; ii++)
		ret += snprintf(buf + ret, PAGE_SIZE, "%02X", read_data[ii]);
...
int synaptics_read_diag_data(struct synaptics_rmi4_data *rmi4_data)
{
	...
	switch (rmi4_data->diag_command) {
		...
		case F54_ABS_HYBRID_RAW_CAP:
			...
			break;
		default:
			dev_info(rmi4_data->pdev->dev.parent," %s: diag_command error\n", __func__);
			break;
	}
			...
			if (rmi4_data->factor_width && rmi4_data->factor_height) {
				if (debug_mask & BIT(1)) {
					dev_info(rmi4_data->pdev->dev.parent,
							...
							__func__, finger,
							finger_status,
							(x * rmi4_data->factor_width) >> SHIFT_BITS,
							(y * rmi4_data->factor_height) >> SHIFT_BITS,
							wx, wy);
				}

				if (debug_mask & BIT(3)) {
					rmi4_data->report_points[finger].x = (x * rmi4_data->factor_width) >> SHIFT_BITS;
					rmi4_data->report_points[finger].y = (y * rmi4_data->factor_height) >> SHIFT_BITS;
...
/* HTC_TP_START, HTC_TP_DMASK */
	if (debug_mask & BIT(2)) {
		...
		dev_info(rmi4_data->pdev->dev.parent, "Touch latency = %ld us\n",
				((time_end.tv_sec * 1000000000 + time_end.tv_nsec) -
				(time_start.tv_sec * 1000000000 + time_start.tv_nsec)) / 1000);
	}
		...
		if (rmi4_data->factor_width && rmi4_data->factor_height) {
			for (finger = 0; finger < fingers_to_process; finger++) {
				if (rmi4_data->report_points[finger].finger_ind != 0) {
					dev_info(rmi4_data->pdev->dev.parent,
							"Screen:F[%02d]:%s, X=%d, Y=%d, Wx=%d, Wy=%d\n",
	...
	if (debug_mask & BIT(4))
		if (rmi4_data->factor_width && rmi4_data->factor_height)
			trace_clock_set_rate("tp_report", (rmi4_data->report_points[0].y * rmi4_data->factor_height) >> SHIFT_BITS, -4);
/* HTC_TP_END, HTC_TP_DMASK */
...
/* HTC_TP_START, HTC_TP_DMASK */
	if (bdata->display_width && bdata->display_height) {
		rmi4_data->factor_width = (bdata->display_width << SHIFT_BITS) / rmi4_data->sensor_max_x;
		rmi4_data->factor_height = (bdata->display_height << SHIFT_BITS) / rmi4_data->sensor_max_y;
	}
/* HTC_TP_END, HTC_TP_DMASK */
...
/* HTC_TP_START, HTC_SENS_HUB */
	if (bdata->switch_gpio >= 0) {
		retval = synaptics_rmi4_gpio_setup(bdata->switch_gpio, true, 1, 0);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
...
/* HTC_TP_START */
	dev_info(rmi4_data->pdev->dev.parent, " %s: enable %d fw_id %d config_id %s\n",
		__func__, enable, rmi4_data->firmware_id, rmi4_data->config_id);
/* HTC_TP_END */
...
void synaptics_rmi4_new_function_v2_6(struct synaptics_rmi4_exp_fn *exp_fn,
		bool insert)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
...
EXPORT_SYMBOL(synaptics_rmi4_new_function_v2_6);
...
/* HTC_TP_START, HTC_SENS_HUB */
static void sensor_hub_switch(struct synaptics_rmi4_data *rmi4_data, int mode)
{
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
...
/* HTC_TP_START, HTC_SENS_HUB */
				sensor_hub_switch(rmi4_data, 1);
/* HTC_TP_END, HTC_SENS_HUB */
				rmi4_data->fb_ready = false;
			} else if (*transition == FB_BLANK_UNBLANK) {
/* HTC_TP_START, HTC_SENS_HUB */
				sensor_hub_switch(rmi4_data, 0);
/* HTC_TP_END, HTC_SENS_HUB */
...
static int __init synaptics_rmi4_init(void)
{
	...
	retval = synaptics_rmi4_bus_init_v2_6();
	if (retval)
		return retval;
