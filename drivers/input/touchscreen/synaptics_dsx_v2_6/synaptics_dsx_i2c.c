...
/* HTC_TP_START, HTC_SENS_HUB */
	...
	if (prop && prop->length) {
		...
		retval = of_property_read_u32(np, "synaptics,switch-hub-state",
				&value);
		if (retval < 0) {
			dev_err(dev, "%s: Unable to read synaptics,switch-hub-state property\n",
					__func__);
			bdata->switch_gpio = -1;
		} else {
			bdata->switch_hub_state = value;
		}
	} else {
		bdata->switch_gpio = -1;
	}
...
/* HTC_TP_START, HTC_TP_DMASK */
	...
	if (prop && prop->length) {
		if (2 == (prop->length / sizeof(u32))) {
			retval = of_property_read_u32_array(np, "synaptics,display-coords",
...
/* HTC_TP_START, HTC_TP_SRC_CTL */
	/* TW PIN ID */
	retval = of_property_read_u32(np, "synaptics,tw-pin-mask", &value);
	if (retval < 0)
		bdata->tw_pin_mask = 0;
	else
	...
	for (i = 0; i < TP_SRC_NUM; i++) {
		retval = of_property_read_string_index(np, "synaptics,tp-src", i,
						  &name);
		if (retval < 0)
			bdata->tp_src[i] = NULL;
		else
	...
	if (prop && prop->length && (prop->length <= TP_SRC_NUM * sizeof(u32))) {
		retval = of_property_read_u32_array(np, "synaptics,tp-src-id",
				bdata->tp_src_id, prop->length / sizeof(u32));
		if (retval < 0)
			memset(bdata->tp_src_id, 0, TP_SRC_NUM * sizeof(uint32_t));
	} else
		memset(bdata->tp_src_id, 0, TP_SRC_NUM * sizeof(uint32_t));
...
static struct of_device_id synaptics_rmi4_of_match_table[] = {
	{
		.compatible = "synaptics_v2_6,dsx-i2c",
	},
	{},
};
...
int synaptics_rmi4_bus_init_v2_6(void)
{
	return i2c_add_driver(&synaptics_rmi4_i2c_driver);
}
EXPORT_SYMBOL(synaptics_rmi4_bus_init_v2_6);

void synaptics_rmi4_bus_exit_v2_6(void)
{
	kfree(wr_buf);
...
EXPORT_SYMBOL(synaptics_rmi4_bus_exit_v2_6);
