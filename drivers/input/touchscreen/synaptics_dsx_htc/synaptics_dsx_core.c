...
#define EXP_FN_WORK_DELAY_MS 500 
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (bdata->tw_pin_mask != 0)
		ret += snprintf(buf+ret, PAGE_SIZE, "_twID-%x", rmi4_data->tw_vendor);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "config_version:%08X\n", rmi4_data->config_version);
...
static int register_sr_touch_device(void)
{
	...
	set_bit(KEY_APP_SWITCH, rmi4_data->sr_input_dev->keybit);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (rmi4_data->sr_input_dev)
	{
		count += sprintf(buf + count, "%s ", rmi4_data->sr_input_dev->name);
...
static int synaptics_rmi4_get_noise_state(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			rmi4_data->f54_data_base_addr + 4,
			data, sizeof(data));
	if (retval < 0)
		return retval;
	im = (data[1] << 8) | data[0];
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			rmi4_data->f54_data_base_addr + 8,
			&ns, sizeof(ns));
	if (retval < 0)
		return retval;

	retval = synaptics_rmi4_reg_read(rmi4_data,
			rmi4_data->f54_data_base_addr + 10,
			data, sizeof(data));
	if (retval < 0)
		return retval;
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			rmi4_data->f54_data_base_addr + 11,
			data, sizeof(data));
	if (retval < 0)
		return retval;
			...
			if (rmi4_data->report_points[finger].state != state) {
				if (debug_mask & BIT(3)) {
					if (rmi4_data->width_factor && rmi4_data->height_factor) {
						pr_info("Screen:%c[%02d]:Down, X=%d, Y=%d, IM=%d, CIDIM=%d, Freq=%d, NS=%d\n",
						...
						pr_info("Raw:%c[%02d]:Down, X=%d, Y=%d, IM=%d, CIDIM=%d, Freq=%d, NS=%d\n",
						state2char(finger_status),
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if (bdata->display_width && bdata->display_height) {
		dev_info(&pdev->dev,"%s Load display resolution: %dx%d\n",
